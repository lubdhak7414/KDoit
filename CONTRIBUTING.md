...
Source lives in `contents/ui/` (`main.qml`, `TaskModel.qml`, `TaskDelegate.qml`, …); config schema in `contents/config/main.xml`. Tasks persist via dual-write: a JSON file at `~/.local/share/kdoit/tasks.json` (for Syncthing / external tools) and a mirror in `plasmoid.configuration.tasksJson` (for fast synchronous startup reads).
